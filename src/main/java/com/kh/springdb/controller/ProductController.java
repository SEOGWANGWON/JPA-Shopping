...
public class ProductController {
	...
	public String productDetail(@PathVariable("id") int id, Model model) {
		...
		//	"product" templates 밑에서 thymeleaf 로 불러올 변수명을 product 로 지정
		//	Product product 만들어준 필드명을 가지고와서 service 로 불러온 내용을
	...
	public String pageList(Model model, @RequestParam(value="page", defaultValue="0") int page) {
		...
		// 만약에 초반이나 후반에 어떤 값이 page 안에 없다면 page 가 null 값 이라면
		// 기본 값으로 0으로 설정해서 초기값을 null 이 아닌 0 으로 처리하겠다
